...
function answerControl(question) {
  ...
  if (question.type === 'stepped') return `<div class="step-list">${question.steps.map((step, index) => `<label class="step-item"><span class="step-index">${index + 1}</span><span class="step-prompt">${renderRichMathText(step.prompt)}</span><input data-exam-step="${question.id}:${step.stepId}" inputmode="decimal" /></label>`).join('')}</div>`;
  if (question.type === 'single-choice') return `<div class="choice-list">${question.options.map((option) => `<label class="choice-option"><input type="radio" name="exam-${question.id}" value="${option}" /><span>${renderRichMathText(option)}</span></label>`).join('')}</div>`;
