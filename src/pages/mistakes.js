...
function mistakeCard(mistake) {
  return `<article class="mistake-card ${mistake.understoodAt ? 'is-understood' : ''}" id="mistake-${mistake.id}" data-mistake-id="${mistake.id}">
...
export function bindMistakesPage() {
  ...
  document.querySelectorAll('[data-mistake-scroll]').forEach((button) => button.addEventListener('click', () => {
    document.getElementById(`mistake-${button.dataset.mistakeScroll}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }));
