...
export function renderLabs() {
  return `<section class="page labs-page">
    ...
    <nav class="lab-jump-nav" aria-label="实验目录"><button type="button" data-lab-scroll="matrix-lab">矩阵乘法</button><button type="button" data-lab-scroll="graph-lab">图论</button><button type="button" data-lab-scroll="probability-lab">概率密度</button><button type="button" data-lab-scroll="markov-lab">天气模拟</button></nav>
    ...
    <section class="lab-section" id="graph-lab"><div class="section-heading"><div><span class="eyebrow">Lab 02</span><h2>图论实验室</h2></div><p>编辑 K4，联动查看 A、D、L、walks 与 spanning trees。</p></div>${renderGraphLab()}</section>
    <section class="lab-section" id="probability-lab"><div class="section-heading"><div><span class="eyebrow">Lab 03</span><h2>概率密度实验室</h2></div><p>把区间概率、期望与方差放回密度曲线下理解。</p></div>${renderProbabilityLab()}</section>
    <section class="lab-section" id="markov-lab"><div class="section-heading"><div><span class="eyebrow">Lab 04</span><h2>马尔可夫天气模拟器</h2></div><p>状态图、转移矩阵与概率曲线同步更新。</p></div>${renderMarkovSimulator()}</section>
  </section>`;
}
